...
namespace Tera.WorldServer.World.Maps
{
    public class Pathfinding
    {
        ...
        public int FreeCellNeightboor(int cell)
        {
            ...
            {
                if (_map.getCell(c).isWalkable(true))
                {
                    free = c;
                    break;
                }
        ...
        public static int sNextCell(int cell, int dir, Map _map)
        {
            switch (dir)
            {
                case 0:
        ...
        public static int RandomJoinCell(int cell, Map map)
        {
            return sNextCell(cell, InLineDirPossible[Rand(0, 3)], map);
        }
        ...
        public static List<int> GetCircleZone(int baseCell, int radius, Map map)
        {
        ...
        public static List<int> NearestCells(Map tMap, int Cell)
        {
            var Cells = new List<int>(4);
            for (int i = 1; (i <= 7); i = (i + 2))
            {
                var nCell = sNextCell(Cell, i, tMap);
                if (tMap.getCell(nCell).Walkable)
                {
                    Cells.Add(nCell);
                }
            }
        ...
        public static List<int> GetLineZone(int baseCell, int lenght, int dir, Map map)
        {
            var cells = new List<int>();
