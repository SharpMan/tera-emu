...
{
    public sealed class CharacterFriendListMessage : PacketBase
    {
        public Player Character;

        public CharacterFriendListMessage(Player perso)
        {
        ...
        public override string Compile()
        {
            ...
            foreach (KeyValuePair<int, String> KeyPair in Character.GetClient().Account.Data.FriendsList)
            {
                sb.Append("|").Append(KeyPair.Value);
                var socket = WorldServer.Network.WorldServer.Clients.FirstOrDefault(x => x.Account.ID == KeyPair.Key);
                if (socket != null && socket.Account != null && socket.Character != null)
                {
                    ...
                    if (socket.Account.Data.FriendsList.ContainsKey(Character.GetClient().Account.ID))
                    {
