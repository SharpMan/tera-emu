...
{
    public sealed class Circulez : Challenge
    {
        public Circulez(Fight Fight)
            : base(21, Fight)
        {
            BasicDropBonus = 10;
            BasicXpBonus = 10;
        ...
        public override void onLaunchWeapon(Fighter Launcher, Database.Models.InventoryItemModel Weapon, int TargetCellId, Fighter TargetFighter, Dictionary<Character.WeaponEffect, List<Fighter>> TargetEffects, bool IsCC, bool IsEchec, bool isPunch)
        {
            ...
            {
                if (TargetEffect.Key.EffectType == EffectEnum.SubPM || TargetEffect.Key.EffectType == EffectEnum.SubPMEsquivable || TargetEffect.Key.EffectType == EffectEnum.VolPM)
                {
